...
def index():
    ...
    ingredientes_raw = conn.execute('SELECT * FROM ingredientes ORDER BY nome ASC').fetchall()
...
# API de auto-update para não precisar de F5
@ingredientes_bp.route('/api/ingredientes/lista', methods=['GET'])
def lista_ingredientes():
    ...
    try:
        ...
        return jsonify({'sucesso': True, 'ingredientes': ingredientes, 'alertas': abaixo_minimo})
...
def entrada_ingrediente():
    ...
    custo_unitario_novo = preco_por_emb / conteudo_por_emb 
